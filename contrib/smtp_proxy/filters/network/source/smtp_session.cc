...
void SmtpSession::updateBytesMeterOnCommand(size_t length) {
  if (length == 0)
    return;

  if (transaction_in_progress_) {
    getTransaction()->getStreamInfo().addBytesReceived(length);
    getTransaction()->getStreamInfo().getDownstreamBytesMeter()->addWireBytesReceived(length);
    getTransaction()->getStreamInfo().getUpstreamBytesMeter()->addHeaderBytesSent(length);

    if (isDataTransferInProgress()) {
      if (bdat_chunk_size_ > 0) {
        getTransaction()->addPayloadBytes(bdat_chunk_size_);
      } else {
        getTransaction()->addPayloadBytes(length);
      }
    }
  }
}

void SmtpSession::updateBytesMeterOnResponse(size_t length) {
  if (length == 0)
    return;

  if (transaction_in_progress_) {
    getTransaction()->getStreamInfo().addBytesSent(length);
    getTransaction()->getStreamInfo().getDownstreamBytesMeter()->addWireBytesSent(length);
    getTransaction()->getStreamInfo().getUpstreamBytesMeter()->addWireBytesReceived(length);
  }
}
...
SmtpUtils::Result SmtpSession::handleCommand(std::string& command, std::string& args) {
  ...
  } else if (absl::EqualsIgnoreCase(command, SmtpUtils::smtpBdatCommand)) {
    result = handleBdat(args);
...
SmtpUtils::Result SmtpSession::handleMail(std::string& arg) {
  ...
  updateBytesMeterOnCommand(callbacks_->getReadBuffer().length());
...
SmtpUtils::Result SmtpSession::handleBdat(std::string& arg) {
  // BDAT command syntax: BDAT <chunk-size> [LAST]
  // Example: BDAT 1024 or BDAT 1024 LAST
  
  if (arg.length() == 0) {
    msg_ = "bad_bdat_cmd_syntax";
    callbacks_->getStats().protocol_parse_error_.inc();
    return SmtpUtils::Result::ProtocolError;
  }
  ...
  if (!transaction_in_progress_ || getTransaction()->getNoOfRecipients() == 0) {
    std::string resp_msg = SmtpUtils::generateResponse(502, {5, 5, 1}, "Missing RCPT TO command");
    setSessionStatus(SmtpUtils::missing_rcpt_cmd, SmtpUtils::local_5xx_error);
    recordLocalRespInSession(SmtpUtils::smtpBdatCommand, 502, resp_msg, SmtpUtils::local_5xx_error);
    callbacks_->sendReplyDownstream(resp_msg);
    callbacks_->getStats().local_5xx_errors_.inc();
    callbacks_->getStats().local_5xx_missing_rcpt_.inc();
    callbacks_->getStats().bad_cmd_sequence_.inc();
  ...
  // Parse BDAT arguments: <chunk-size> [LAST]
  std::vector<std::string> parts = absl::StrSplit(arg, ' ');
  ...
  bool is_last_chunk = (parts.size() == 2 && absl::EqualsIgnoreCase(parts[1], "LAST"));
  
  newCommand(SmtpUtils::smtpBdatCommand, SmtpCommand::Type::TransactionCommand);
  setTransactionState(SmtpTransaction::State::TransactionInProgress);
  setDataTransferInProgress(true);
  updateBytesMeterOnCommand(callbacks_->getReadBuffer().length());
  
  // Store BDAT parameters for later use
  bdat_chunk_size_ = chunk_size;
  bdat_is_last_chunk_ = is_last_chunk;
...
SmtpUtils::Result SmtpSession::handleResponse(int& response_code, std::string& response) {
  ...
  if (absl::EqualsIgnoreCase(command, SmtpUtils::smtpEhloCommand) ||
    ...
    result = handleBdatResponse(response_code, response);
  } else if (absl::EqualsIgnoreCase(command, SmtpUtils::smtpAuthCommand)) {
...
SmtpUtils::Result SmtpSession::handleMailResponse(int& response_code, std::string& response) {
  ...
  storeResponse(response, SmtpUtils::via_upstream, response_code,
                SmtpCommand::ResponseType::ViaUpstream);
  updateBytesMeterOnResponse(callbacks_->getWriteBuffer().length());
  if (response_code == 250) {
    setTransactionState(SmtpTransaction::State::TransactionInProgress);
    ...
    if (callbacks_->tracingEnabled() && !getTransaction()->isXReqIdSent()) {
      ...
      Buffer::OwnedImpl data(x_req_id);
      newCommand(SmtpUtils::xReqIdCommand, SmtpCommand::Type::NonTransactionCommand);
      updateBytesMeterOnCommand(data.length());
      callbacks_->sendUpstream(data);
...
SmtpUtils::Result SmtpSession::handleBdatResponse(int& response_code, std::string& response) {
  SmtpUtils::Result result = SmtpUtils::Result::ReadyForNext;
  storeResponse(response, SmtpUtils::via_upstream, response_code,
                SmtpCommand::ResponseType::ViaUpstream);
  updateBytesMeterOnResponse(callbacks_->getWriteBuffer().length());
  ...
  if (response_code == 250) {
    // BDAT chunk accepted
    if (bdat_is_last_chunk_) {
      // This was the last chunk, transaction is complete
      setTransactionState(SmtpTransaction::State::TransactionCompleted);
      getTransaction()->setStatus(SmtpUtils::statusSuccess);
      onTransactionComplete();
      if (data_tx_length_ != nullptr) {
      ...
      // More chunks expected, stay in transaction state
      setTransactionState(SmtpTransaction::State::TransactionInProgress);
    }
  } else if (response_code >= 400 && response_code <= 499) {
    // Error response to BDAT command
    getTransaction()->setMsg(response);
    callbacks_->getStats().upstream_4xx_data_.inc();
    getTransaction()->setStatus("upstream_4xx_bdat");
    setTransactionState(SmtpTransaction::State::None);
    onTransactionFailed(response);
  } else if (response_code >= 500 && response_code <= 599) {
    getTransaction()->setMsg(response);
    callbacks_->getStats().upstream_5xx_data_.inc();
    getTransaction()->setStatus("upstream_5xx_bdat");
    setTransactionState(SmtpTransaction::State::None);
    onTransactionFailed(response);
  }

  // Reset BDAT state for next chunk
  bdat_chunk_size_ = 0;
  bdat_is_last_chunk_ = false;
