...
public:
  ...
  SmtpUtils::Result handleBdat(std::string& args);
  ...
  SmtpUtils::Result handleBdatResponse(int& response_code, std::string& response);
  ...
  void setAuthCompletionStatus(bool status) { auth_complete_ = status; }
  bool isAuthInProgress() { return auth_in_progress_; }
  void setAuthInProgress(bool status) { auth_in_progress_ = status; }

  std::shared_ptr<SmtpCommand> getCurrentCommand() { return current_command_; }
  void updateBytesMeterOnCommand(size_t length);
  void updateBytesMeterOnResponse(size_t length);
...
private:
  ...
  int bdat_chunk_size_{0};
  bool bdat_is_last_chunk_{false};
