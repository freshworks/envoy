...
TEST_F(DecoderImplTest, TestParseCommand) {
  // When the session state is SESSION_IN_PROGRESS and an invalid command is received, it is passed to upstream, without any state change.
  ...
  decoder_impl_->getSession().setState(SmtpSession::State::SESSION_IN_PROGRESS);
  EXPECT_EQ(Decoder::Result::ReadyForNext, decoder_impl_->parseCommand(*data_));
  EXPECT_EQ(SmtpSession::State::SESSION_TERMINATION_REQUEST, decoder_impl_->getSession().getState());

  //When the session state is SESSION_TERMINATED and any command is received, filter drops the request.
  data_->drain(data_->length());
...
TEST_F(DecoderImplTest, TestSmtpTransactionCommands) {
  ...
  EXPECT_EQ(decoder_impl_->getSession().getTransactionState(), SmtpTransaction::State::TRANSACTION_REQUEST);
  ...
  decoder_impl_->decodeSmtpTransactionCommands(command);
  EXPECT_EQ(decoder_impl_->getSession().getTransactionState(), SmtpTransaction::State::RCPT_COMMAND);
  ...
  decoder_impl_->getSession().SetTransactionState(SmtpTransaction::State::TRANSACTION_IN_PROGRESS);
  decoder_impl_->decodeSmtpTransactionCommands(command);
  EXPECT_EQ(decoder_impl_->getSession().getTransactionState(), SmtpTransaction::State::MAIL_DATA_TRANSFER_REQUEST);
  
  // Transaction abort/reset
  command = SmtpUtils::smtpRsetCommand;
  decoder_impl_->getSession().SetTransactionState(SmtpTransaction::State::TRANSACTION_IN_PROGRESS);
  decoder_impl_->decodeSmtpTransactionCommands(command);
  EXPECT_EQ(decoder_impl_->getSession().getTransactionState(), SmtpTransaction::State::TRANSACTION_ABORT_REQUEST);
  
  decoder_impl_->getSession().SetTransactionState(SmtpTransaction::State::MAIL_DATA_TRANSFER_REQUEST);
  decoder_impl_->decodeSmtpTransactionCommands(command);
  EXPECT_EQ(decoder_impl_->getSession().getTransactionState(), SmtpTransaction::State::TRANSACTION_ABORT_REQUEST);
...
TEST_F(DecoderImplTest, TestParseResponse) {
  ...
  // If quit command receives non 2xx response from server, session state is set back to in-progress.
  decoder_impl_->getSession().setState(SmtpSession::State::SESSION_TERMINATION_REQUEST);
  data_->drain(data_->length());
  data_->add("500"); 
...
TEST_F(DecoderImplTest, TestDecodeSmtpTransactionResponse) {
  ...
  EXPECT_EQ( decoder_impl_->getSession().getTransactionState(), SmtpTransaction::State::TRANSACTION_IN_PROGRESS);
  ...
  decoder_impl_->decodeSmtpTransactionResponse(response_code);
  EXPECT_EQ( decoder_impl_->getSession().getTransactionState(), SmtpTransaction::State::TRANSACTION_COMPLETED);
  testing::Mock::VerifyAndClearExpectations(&callbacks_);
  ...
  decoder_impl_->getSession().SetTransactionState(SmtpTransaction::State::TRANSACTION_ABORT_REQUEST);
  response_code = 250;
  EXPECT_CALL(callbacks_, incSmtpTransactionsAborted());
  decoder_impl_->decodeSmtpTransactionResponse(response_code);
  EXPECT_EQ( decoder_impl_->getSession().getTransactionState(), SmtpTransaction::State::NONE);
