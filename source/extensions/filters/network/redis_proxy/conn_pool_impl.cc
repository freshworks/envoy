...
InstanceImpl::ThreadLocalPool::makeRequest(const std::string& key, RespVariant&& request,
  ...
  if ((transaction.active_ && transaction.is_transaction_mode_)&& !transaction.connection_established_) {
    transaction.clients_[client_idx] =
        client_factory_.create(host, dispatcher_, *config_, redis_command_stats_, *(stats_scope_),
                               auth_username_, auth_password_, true,false,false,nullptr);
...
// If shard index is negative its a blocking command with a key else its a pubsub command
  ...
  if (transaction.active_) {
    ...
    // Check if the client is not empty before creating a new connection
    if (transaction.clients_[client_idx] == nullptr) {
        // Determine the connection parameters based on the transaction type
        bool is_pubsub_mode = transaction.isSubscribedMode();
        bool is_blocking_command = transaction.isBlockingCommand();
        
        // Create a new connection based on the transaction type
        transaction.clients_[client_idx] =
            client_factory_.create(host, dispatcher_, *config_, redis_command_stats_, *(stats_scope_),
                                   auth_username_, auth_password_, false, is_pubsub_mode, is_blocking_command, transaction.getDownstreamCallback());
        
        // Add connection callbacks if available
        if (transaction.connection_cb_) {
            transaction.clients_[client_idx]->addConnectionCallbacks(*transaction.connection_cb_);
        }
    }
